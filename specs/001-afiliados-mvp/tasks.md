# Tareas iniciales

- [ ] T1. Resolver pendientes de clarificación y fijar el primer segmento.
- [ ] T2. Crear inventario inicial de categorías, productos y fuentes.
- [ ] T3. Diseñar journey principal y wireframe responsive.
- [ ] T2d. Convertir la matriz de productos en fichas individuales verificadas.
- [ ] T2e. Completar la plantilla editorial y checklist de cada producto de la primera tanda.
- [ ] T2a. Validar el diagnóstico de cinco preguntas y sus reglas editoriales.
- [ ] T2b. Crear una ficha de cumplimiento para cada producto antes de publicarlo.
- [ ] T2c. Definir eventos y métricas del embudo.
- [ ] T3. Diseñar journey principal y wireframe responsive.
- [ ] T3a. Crear CPT `herramientas`, taxonomías y campos internos/editoriales.
- [ ] T3b. Crear plantilla WordPress para landing de producto.
- [x] T3c. Instalar y probar el plugin `brujula-digital-catalog` en un WordPress de staging.
- [ ] T4. Implementar modelo de catálogo y estados de verificación.
- [ ] T5. Implementar landing, selector de dolor, resultados y ficha.
- [ ] T6. Añadir disclosures, privacidad, accesibilidad y SEO base.
- [ ] T7. Instrumentar embudo y definir KPIs.
- [ ] T8. Ejecutar QA funcional, visual, enlaces y rendimiento.
- [ ] T9. Preparar decisión de publicación; no desplegar sin autorización.
## Estado verificado — 2026-09-16

- T1: En progreso. Segmento inicial definido como profesionales independientes y pequeñas empresas; falta cerrar el primer catálogo operativo.
- T2: En progreso. Existe matriz inicial y se ha documentado Pipedrive con fuentes oficiales; faltan las restantes fichas con verificación actual.
- T3: En progreso. El flujo inicio → categoría → ficha → CTA está probado en staging y el diagnóstico local de tres pasos se ha recorrido en navegador; falta integrarlo en WordPress.
- T4: En progreso. El CPT y los estados editoriales existen en WordPress; se ha añadido un esquema JSON para separar datos de producto, editorial y afiliación.
- T5: En progreso. AIHomeDesign está publicado y Pipedrive tiene landing especificada; el prototipo y el shortcode del diagnóstico están preparados, pero Pipedrive no está cargado como ficha publicable.
- T6: Parcialmente verificado. Disclosure y CTA etiquetado están presentes en AIHomeDesign; privacidad, contraste y auditoría completa requieren revisión adicional.
- T7: Parcialmente verificado. El evento `affiliate_click` está implementado; faltan baseline, consentimiento/tratamiento de analítica y objetivos por categoría.
- T8: Parcialmente verificado. Se ha comprobado el flujo público y el catálogo filtrado; falta prueba completa de teclado, móvil real, estados de error y rendimiento.
- T9: Pendiente. No publicar Pipedrive sin aprobación del programa, enlace de afiliado y revisión de términos.
